...
for face in faces:
    ...
    for t in triangles:
        ...
        index_pt1 = extract_index_np_array(index_pt1)

        index_pt2 = np.where((points == pt2).all(axis=1))
        index_pt2 = extract_index_np_array(index_pt2)

        index_pt3 = np.where((points == pt3).all(axis=1))
        index_pt3 = extract_index_np_array(index_pt3)
...
img2_head_no_face = np.zeros_like(img)
img2_head_no_face = cv2.bitwise_and(img2, img2, mask=img2_face_mask)
result = cv2.add(img2_head_no_face, img2_new_face)
